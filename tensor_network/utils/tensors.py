...
# %% ../../1-4.ipynb #initial_id
import torch

# %% ../../1-4.ipynb #f8b905cff34fb2d0
def identity_tensor(
    order: int,
    dim: int,
    ...
    Args:
        ...
        device (torch.device | None): The device to create the tensor on. Default is None.
    Returns:
        torch.Tensor: The identity tensor of shape (dim, dim, ..., dim) with the specified order.
    """
    dims = [dim] * order
    I = torch.zeros(*dims, dtype=dtype, device=device)
    for i in range(dim):
        indices = [i] * order
        I[tuple(indices)] = 1.0
...
# %% ../../3-1.ipynb #2fb1e121
def zeros_state(
    *, num_qubits: int, dtype: torch.dtype, device: torch.device | None = None
) -> torch.Tensor:
    """
