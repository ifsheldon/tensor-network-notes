...
def load_mnist_images(
    *,
    ...
    if classes is not None:
        ...
        mask = torch.isin(labels, torch.tensor(classes, device=labels.device))
        images = images[mask]
        labels = labels[mask]
...
def split_classification_dataset(
    ...
    assert data.device == targets.device, "data and targets must be on the same device"
    ...
    for class_idx in range(num_classes):
        ...
        if shuffle:
            shuffle_idx = torch.randperm(num_samples_class_i, device=device)
            ...
            train_labels.append(
                torch.full((train_sample.shape[0],), class_idx, dtype=targets.dtype, device=device)
            )
            test_labels.append(torch.full((test_sample.shape[0],), class_idx, dtype=targets.dtype, device=device))
        else:
