...
def apply_gate(
    *,
    ...
    gate = gate.to(device=quantum_state.device)
...
def pauli_operator(
    *,
    ...
    Args:
        ...
        device (torch.device | None): The device to move the tensor to. If None, return a CPU tensor.
    Returns:
        torch.Tensor: The Pauli operator as a tensor.
    """
    operator = _pauli_operator_base(
        pauli=pauli, double_precision=double_precision, force_complex=force_complex
    )
    if device is None:
        return operator
    return operator.to(device=device)
...
def get_control_gate_tensor(
    ...
    *,
    ...
    applied_gate = applied_gate.to(dtype=dtype, device=device)
...
def rand_unitary(
    ...
    work_device = linalg_work_device(target_device)
    # MPS QR is not consistently available, so only MPS falls back to CPU.
    mat = torch.randn(dim, dim, dtype=dtype, device=work_device)
    Q, _R = torch.linalg.qr(mat)
    return Q.to(device=target_device)
