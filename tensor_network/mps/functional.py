...
def orthogonalize_left2right_step(
    ...
    original_device = view_matrix.device
    work_device = linalg_work_device(original_device)
    # MPS linalg on MPS is incomplete, so only that backend falls back to CPU.
    view_matrix_work = view_matrix.to(work_device)

    if mode == "svd":
        u, lm, v = torch.linalg.svd(view_matrix_work, full_matrices=False)
        u = u.to(original_device)
        lm = lm.to(original_device)
        ...
        u, r = torch.linalg.qr(view_matrix_work)
        u = u.to(original_device)
        r = r.to(original_device)
