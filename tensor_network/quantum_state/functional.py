...
def calc_observation(
    ...
    operator = operator.to(device=state.device)
...
def calc_onsite_entanglement_entropy(
    ...
    for idx in qubit_idx:
        ...
        work_device = linalg_work_device(rdm.device)
        # MPS eigvalsh is not consistently available, so only MPS falls back to CPU.
        eigvals = torch.linalg.eigvalsh(rdm.to(work_device)).to(state.device)
...
def project_state(
    ...
    project_qubit_state = project_qubit_state.to(device=state.device, dtype=state.dtype)
...
def observe_bond_energies(
    ...
    for h, pos in zip(hamiltonian, interaction_positions):
        h = h.to(device=quantum_state.device)
...
def bipartite_entanglement_entropy(quantum_state: torch.Tensor, qubit_idx: int | List[int] | None):
    ...
    for idx in indices:
        ...
        work_device = linalg_work_device(mat.device)
        # MPS svdvals is not consistently available, so only MPS falls back to CPU.
        eigenvalues = torch.linalg.svdvals(mat.to(work_device)).to(quantum_state.device) ** 2
