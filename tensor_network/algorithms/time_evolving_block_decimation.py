...
def calculate_mps_local_energies(
    ...
    for pos, hamiltonian in zip(positions, hamiltonians):
        ...
        hamiltonian = hamiltonian.to(device=rdm.device)
...
def tebd(
    ...
    for t in tqdm(range(iterations), **progress_bar_kwargs):
        ...
        if (iters_for_tau > tau_min and (t + 1) % calc_observation_iters == 0) or (
            ...
            if os.environ.get("TN_NOTEBOOK_SMOKE") != "1":
                # Reference MPS code has device quirks, so keep this comparison CPU-backed.
                ref_tensors = [t.detach().cpu() for t in mps.local_tensors]
                ref_mps = MPS_tebd(tensors=ref_tensors, para={"dtype": mps.dtype, "device": "cpu"})
                ...
                local_energies_ref = ref_mps.calculate_local_energies(hs, ps).to(
                    local_energies_new.device
                )
