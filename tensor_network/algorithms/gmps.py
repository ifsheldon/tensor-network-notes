...
def eval_nll(
    *,
    ...
    env_vector_left = torch.ones(
        batch_size, left_virtual_dim, dtype=samples.dtype, device=device
    )
    right_virtual_dim = mps_local_tensors[-1].shape[-1]
    env_vector_right = torch.ones(
        batch_size, right_virtual_dim, dtype=samples.dtype, device=device
    )
...
def _batch_index_splits(dataset_size: int, batch_size: int, device: torch.device):
    # MPS random indexing is not consistently supported, so create indices on CPU there.
    index_device = torch.device("cpu") if device.type == "mps" else device
    permutation = torch.randperm(dataset_size, device=index_device)
    if permutation.device != device:
        permutation = permutation.to(device=device)
    return permutation.split(batch_size)
    ...
    *,
    ...
    for i in progress_bar:
        ...
        for batch_indices in tqdm(
            _batch_index_splits(dataset_size, batch_size, device),
            leave=False,
            disable=disable_batch_progress_bar,
        ):
            batch_data = samples[batch_indices]
            current_batch_size = batch_data.shape[0]
            # prepare aux variables
            env_vectors_left: List[torch.Tensor | None] = [None] * mps.length
            env_vectors_left[0] = torch.ones(
                current_batch_size, left_virtual_dim, dtype=samples.dtype, device=device
            )
            env_vectors_right: List[torch.Tensor | None] = [None] * mps.length
            env_vectors_right[-1] = torch.ones(
                current_batch_size, right_virtual_dim, dtype=samples.dtype, device=device
            )
            norm_factors = torch.ones(
                current_batch_size, feature_num, dtype=samples.dtype, device=device
            )
...
def prepend_labels(raw_images: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    ...
    labels = labels.to(device=raw_images.device)
...
def eval_nll_selected_features(
    *,
    ...
    mps_local_tensors = mps.local_tensors
    batch_size = dataset_size  # since we do the init NLL evaluation in one go
    env_vectors_left = torch.ones(batch_size, 1, 1, dtype=samples.dtype, device=device)
    env_vectors_right = torch.ones(batch_size, 1, 1, dtype=samples.dtype, device=device)
    norm_factors = torch.ones(batch_size, feature_num, dtype=samples.dtype, device=device)
